...
static int set_int_enable(int fd, unsigned char enable);
...
#ifdef AK89xx_SECONDARY
static int setup_compass(int fd);
...
static int set_int_enable(int fd, unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on) {
        if (enable)
            ...
            tmp = 0x00;
        if (i2c_write( fd, st.hw->addr, st.reg->int_enable, 1, &tmp))
            return -1;
...
int mpu_reg_dump(int fd)
{
    ...
    for (ii = 0; ii < st.hw->num_reg; ii++) {
        if (ii == st.reg->fifo_r_w || ii == st.reg->mem_r_w)
            continue;
        if (i2c_read( fd, st.hw->addr, ii, 1, &data))
            return -1;
        log_i("%#5x: %#5x\r\n", ii, data);
    }
    return 0;
...
int mpu_read_reg(int fd, unsigned char reg, unsigned char *data)
{
    ...
    if (reg >= st.hw->num_reg)
        return -1;
    return i2c_read( fd, st.hw->addr, reg, 1, data);
}
...
int mpu_init(int fd, struct int_param_s *int_param)
{
    ...
    /* Reset device. */
    data[0] = BIT_RESET;
    if (i2c_write( fd, st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    delay_ms(100);
   ...
   st.chip_cfg.accel_half = 0;
    ...
    if (i2c_write( fd, st.hw->addr, st.reg->accel_cfg2, 1, data))
        return -1;
#endif
    ...
    if (mpu_set_gyro_fsr(fd,2000))
        return -1;
    if (mpu_set_accel_fsr(fd,2))
        return -1;
    if (mpu_set_lpf(fd,42))
        return -1;
    if (mpu_set_sample_rate(fd,50))
        return -1;
    if (mpu_configure_fifo(fd,0))
        return -1;
...
#ifdef AK89xx_SECONDARY
    setup_compass(fd);
    if (mpu_set_compass_sample_rate(fd,10))
        return -1;
#else
...
int mpu_lp_accel_mode(int fd, unsigned short rate)
{
    ...
    if (!rate) {
        mpu_set_int_latched(fd,0);
        tmp[0] = 0;
        tmp[1] = BIT_STBY_XYZG;
        if (i2c_write( fd, st.hw->addr, st.reg->pwr_mgmt_1, 2, tmp))
            return -1;
        st.chip_cfg.lp_accel_mode = 0;
        return 0;
    }
    ...
    if (rate == 1)
        ...
        tmp[0] = INV_LPA_640HZ;
    if (i2c_write( fd, st.hw->addr, st.reg->lp_accel_odr, 1, tmp))
        return -1;
    /* Enable LP ACCEL mode, ACCEL_FCHOICE_B=1*/
    if (i2c_read( fd, st.hw->addr, st.reg->accel_cfg2, 1, tmp))
       	return -1;

    tmp[0] = BIT_ACCL_FC_B | tmp[0];
    if (i2c_write( fd, st.hw->addr, st.reg->accel_cfg2, 1, tmp))
    	return -1;
    tmp[0] = BIT_LPA_CYCLE;
    if (i2c_write( fd, st.hw->addr, st.reg->pwr_mgmt_1, 1, tmp))
        return -1;
#endif
    st.chip_cfg.sensors = INV_XYZ_ACCEL;
    st.chip_cfg.clk_src = 0;
    st.chip_cfg.lp_accel_mode = 1;
    mpu_configure_fifo(fd, 0);
...
int mpu_get_gyro_reg(int fd, short *data, unsigned long *timestamp)
{
    ...
    if (i2c_read( fd, st.hw->addr, st.reg->raw_gyro, 6, tmp))
        return -1;
...
int mpu_get_accel_reg(int fd, short *data, unsigned long *timestamp)
{
    ...
    if (i2c_read( fd, st.hw->addr, st.reg->raw_accel, 6, tmp))
        return -1;
    data[0] = (tmp[0] << 8) | tmp[1];
    data[1] = (tmp[2] << 8) | tmp[3];
    data[2] = (tmp[4] << 8) | tmp[5];
...
int mpu_get_temperature(int fd, long *data, unsigned long *timestamp)
{
    ...
    if (i2c_read( fd, st.hw->addr, st.reg->temp, 2, tmp))
        return -1;
    raw = (tmp[0] << 8) | tmp[1];
    if (timestamp)
        get_ms(timestamp);
...
int mpu_read_6500_accel_bias(int fd, long *accel_bias) {
	unsigned char data[6];
	if (i2c_read( fd, st.hw->addr, 0x77, 2, &data[0]))
		return -1;
	if (i2c_read( fd, st.hw->addr, 0x7A, 2, &data[2]))
		return -1;
	if (i2c_read( fd, st.hw->addr, 0x7D, 2, &data[4]))
		return -1;
...
int mpu_read_6050_accel_bias(int fd, long *accel_bias) {
	unsigned char data[6];
	if (i2c_read( fd, st.hw->addr, 0x06, 2, &data[0]))
		return -1;
	if (i2c_read( fd, st.hw->addr, 0x08, 2, &data[2]))
		return -1;
	if (i2c_read( fd, st.hw->addr, 0x0A, 2, &data[4]))
		return -1;
	accel_bias[0] = ((long)data[0]<<8) | data[1];
	accel_bias[1] = ((long)data[2]<<8) | data[3];
	accel_bias[2] = ((long)data[4]<<8) | data[5];
...
int mpu_read_6500_gyro_bias(int fd, long *gyro_bias) {
	unsigned char data[6];
	if (i2c_read( fd, st.hw->addr, 0x13, 2, &data[0]))
		return -1;
	if (i2c_read( fd, st.hw->addr, 0x15, 2, &data[2]))
		return -1;
	if (i2c_read( fd, st.hw->addr, 0x17, 2, &data[4]))
		return -1;
...
int mpu_set_gyro_bias_reg(int fd, long *gyro_bias)
{
    ...
    if (i2c_write( fd, st.hw->addr, 0x13, 2, &data[0]))
        return -1;
    if (i2c_write( fd, st.hw->addr, 0x15, 2, &data[2]))
        return -1;
    if (i2c_write( fd, st.hw->addr, 0x17, 2, &data[4]))
        return -1;
    return 0;
}
...
int mpu_set_accel_bias_6050_reg(int fd, const long *accel_bias) {
    ...
    if(mpu_read_6050_accel_bias(fd, accel_reg_bias))
        return -1;
    ...
    if (i2c_write( fd, st.hw->addr, 0x06, 2, &data[0]))
        return -1;
    if (i2c_write( fd, st.hw->addr, 0x08, 2, &data[2]))
        return -1;
    if (i2c_write( fd, st.hw->addr, 0x0A, 2, &data[4]))
        return -1;
...
int mpu_set_accel_bias_6500_reg(int fd, const long *accel_bias) {
    ...
    if(mpu_read_6500_accel_bias(fd, accel_reg_bias))
        return -1;
    ...
    if (i2c_write( fd, st.hw->addr, 0x77, 2, &data[0]))
        return -1;
    if (i2c_write( fd, st.hw->addr, 0x7A, 2, &data[2]))
        return -1;
    if (i2c_write( fd, st.hw->addr, 0x7D, 2, &data[4]))
        return -1;
...
int mpu_reset_fifo(int fd)
{
    ...
    data = 0;
    if (i2c_write( fd, st.hw->addr, st.reg->int_enable, 1, &data))
        return -1;
    if (i2c_write( fd, st.hw->addr, st.reg->fifo_en, 1, &data))
        return -1;
    if (i2c_write( fd, st.hw->addr, st.reg->user_ctrl, 1, &data))
        return -1;
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (i2c_write( fd, st.hw->addr, st.reg->int_enable, 1, &data))
            return -1;
        if (i2c_write( fd, st.hw->addr, st.reg->fifo_en, 1, &st.chip_cfg.fifo_enable))
            return -1;
    }
    return 0;
}
...
int mpu_set_gyro_fsr(int fd, unsigned short fsr)
{
    ...
    if (st.chip_cfg.gyro_fsr == (data >> 3))
        return 0;
    if (i2c_write( fd, st.hw->addr, st.reg->gyro_cfg, 1, &data))
        return -1;
...
int mpu_set_accel_fsr(int fd, unsigned char fsr)
{
    ...
    if (st.chip_cfg.accel_fsr == (data >> 3))
        return 0;
    if (i2c_write( fd, st.hw->addr, st.reg->accel_cfg, 1, &data))
        return -1;
...
int mpu_set_lpf(int fd, unsigned short lpf)
{
    ...
    if (st.chip_cfg.lpf == data)
        return 0;
    if (i2c_write( fd, st.hw->addr, st.reg->lpf, 1, &data))
        return -1;
#ifdef MPU6500
...
int mpu_set_sample_rate(int fd, unsigned short rate)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        if (st.chip_cfg.lp_accel_mode) {
	        ...
	        mpu_lp_accel_mode(fd, rate);
                return 0;
            }
            /* Requested rate exceeds the allowed frequencies in LP accel mode,
             * switch back to full-power mode.
             */
            mpu_lp_accel_mode(fd, 0);
        }
        if (rate < 4)
            rate = 4;
        else if (rate > 1000)
        ...
        data = 1000 / rate - 1;
        if (i2c_write( fd, st.hw->addr, st.reg->rate_div, 1, &data))
            return -1;

        st.chip_cfg.sample_rate = 1000 / (1 + data);

#ifdef AK89xx_SECONDARY
        mpu_set_compass_sample_rate( fd, min(st.chip_cfg.compass_sample_rate, MAX_COMPASS_SAMPLE_RATE) );
#endif

        /* Automatically set LPF to 1/2 sampling rate. */
        mpu_set_lpf(fd, st.chip_cfg.sample_rate >> 1);
        return 0;
    }
}
...
int mpu_set_compass_sample_rate(int fd, unsigned short rate)
{
    ...
    if (i2c_write( fd, st.hw->addr, st.reg->s4_ctrl, 1, &div))
        return -1;
...
int mpu_set_sensors(int fd, unsigned char sensors)
{
    ...
    if (!(sensors & INV_XYZ_ACCEL))
        data |= BIT_STBY_XYZA;
    if (i2c_write( fd, st.hw->addr, st.reg->pwr_mgmt_2, 1, &data)) {
        st.chip_cfg.sensors = 0;
        return -1;
    }
    ...
    if (sensors & INV_XYZ_COMPASS)
        ...
        mpu_set_bypass(0);
#else
    if (i2c_read( fd, st.hw->addr, st.reg->user_ctrl, 1, &user_ctrl))
        return -1;
    ...
    if (st.chip_cfg.dmp_on)
        ...
        user_ctrl &= ~BIT_DMP_EN;
    if (i2c_write( fd, st.hw->addr, st.reg->s1_do, 1, &data))
        return -1;
    /* Enable/disable I2C master mode. */
    if (i2c_write( fd, st.hw->addr, st.reg->user_ctrl, 1, &user_ctrl))
        return -1;
#endif
#endif
...
int mpu_get_int_status(int fd, short *status)
{
    ...
    if (!st.chip_cfg.sensors)
        return -1;
    if (i2c_read( fd, st.hw->addr, st.reg->dmp_int_status, 2, tmp))
        return -1;
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    if (i2c_read( fd, st.hw->addr, st.reg->fifo_count_h, 2, data))
        return -1;
...
//    log_i("FIFO count: %hd\n", fifo_count);
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (i2c_read( fd, st.hw->addr, st.reg->int_status, 1, data))
            return -1;
    ...
    if (i2c_read( fd, st.hw->addr, st.reg->fifo_r_w, packet_size, data))
        return -1;
    ...
    unsigned char *more)
{
    ...
    if (i2c_read( fd, st.hw->addr, st.reg->fifo_count_h, 2, tmp))
        return -1;
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        /* FIFO is 50% full, better check overflow bit. */
        if (i2c_read( fd, st.hw->addr, st.reg->int_status, 1, tmp))
            return -1;
    ...
    if (i2c_read( fd, st.hw->addr, st.reg->fifo_r_w, length, data))
        return -1;
...
int mpu_set_bypass(int fd, unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        if (i2c_read( fd, st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        tmp &= ~BIT_AUX_IF_EN;
        if (i2c_write( fd, st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        delay_ms(3);
        tmp = BIT_BYPASS_EN;
        ...
        if (i2c_write( fd, st.hw->addr, st.reg->int_pin_cfg, 1, &tmp))
            return -1;
    } else {
...
#ifdef MPU6050
static int get_accel_prod_shift(int fd, float *st_shift)
{
    unsigned char tmp[4], shift_code[3], ii;

    if (i2c_read( fd, st.hw->addr, 0x0D, 4, tmp))
        return 0x07;
...
static int accel_self_test(int fd, long *bias_regular, long *bias_st)
{
    int jj, result = 0;
    float st_shift[3], st_shift_cust, st_shift_var;
    float accel_max_z_bias, accel_max_xy_bias;

    get_accel_prod_shift(fd, st_shift);
    for(jj = 0; jj < 3; jj++) {
...
static int gyro_self_test(int fd, long *bias_regular, long *bias_st)
{
    int jj, result = 0;
    unsigned char tmp[3];
    ...
    if (i2c_read( fd, st.hw->addr, 0x0D, 3, tmp))
        return 0x07;
...
#endif 
#ifdef AK89xx_SECONDARY
static int compass_self_test(int fd)
{
    unsigned char tmp[6];
    ...
    if (i2c_write( fd, st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp))
        return 0x07;
    tmp[0] = AKM_BIT_SELF_TEST;
    if (i2c_write( fd, st.chip_cfg.compass_addr, AKM_REG_ASTC, 1, tmp))
        goto AKM_restore;
    ...
    do {
        delay_ms(10);
        if (i2c_read( fd, st.chip_cfg.compass_addr, AKM_REG_ST1, 1, tmp))
            goto AKM_restore;
    ...
    if (i2c_read( fd, st.chip_cfg.compass_addr, AKM_REG_HXL, 6, tmp))
        goto AKM_restore;
...
AKM_restore:
    ...
    i2c_write( fd, st.chip_cfg.compass_addr, AKM_REG_ASTC, 1, tmp);
    tmp[0] = SUPPORTS_AK89xx_HIGH_SENS;
    i2c_write( fd, st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp);
    mpu_set_bypass(fd, 0);
    return result;
}
#endif

static int get_st_biases(int fd, long *gyro, long *accel, unsigned char hw_test)
{
    unsigned char data[MAX_PACKET_LENGTH];
    ...
    data[0] = 0x01;
    data[1] = 0;
    if (i2c_write( fd, st.hw->addr, st.reg->pwr_mgmt_1, 2, data))
        return -1;
    delay_ms(200);
    data[0] = 0;
    if (i2c_write( fd, st.hw->addr, st.reg->int_enable, 1, data))
    ...
    if (i2c_write( fd, st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    if (i2c_write( fd, st.hw->addr, st.reg->i2c_mst, 1, data))
        return -1;
    if (i2c_write( fd, st.hw->addr, st.reg->user_ctrl, 1, data))
        return -1;
    ...
    for (ii = 0; ii < packet_count; ii++) {
        ...
        if (i2c_read( fd, st.hw->addr, st.reg->fifo_r_w, MAX_PACKET_LENGTH, data))
            return -1;
...
static int accel_6500_self_test( int fd, long *bias_regular, long *bias_st, int debug)
{
    ...
    if (i2c_read( fd, st.hw->addr, REG_6500_XA_ST_DATA, 3, regs)) {
    	if(debug)
...
static int gyro_6500_self_test(int fd, long *bias_regular, long *bias_st, int debug)
{
    int i, result = 0, otp_value_zero = 0;
    ...
    if (i2c_read( fd, st.hw->addr, REG_6500_XG_ST_DATA, 3, regs)) {
    	if(debug)
    		log_i("Reading OTP Register Error.\n");
        return 0x07;
    }
...
static int get_st_6500_biases(int fd, long *gyro, long *accel, unsigned char hw_test, int debug)
{
    ...
    while (s < test.packet_thresh) {
		...
		if (i2c_read( fd, st.hw->addr, st.reg->fifo_r_w, read_size, data))
						return -1;
		ind = 0;
		for (ii = 0; ii < packet_count; ii++) {
			short accel_cur[3], gyro_cur[3];
...
int mpu_run_self_test(int fd, long *gyro, long *accel)
{
...
#if defined MPU6050
    for (ii = 0; ii < tries; ii++)
        if (!get_st_biases(fd, gyro, accel, 0))
            break;
    if (ii == tries) {
        /* If we reach this point, we most likely encountered an I2C error.
         * We'll just report an error for all three sensors.
    ...
    for (ii = 0; ii < tries; ii++)
        if (!get_st_biases(fd, gyro_st, accel_st, 1))
            break;
    if (ii == tries) {
        /* Again, probably an I2C error. */
        result = 0;
    ...
    accel_result = accel_self_test(fd, accel, accel_st);
    gyro_result = gyro_self_test(fd, gyro, gyro_st);
        ...
        unsigned char *data)
{
    ...
    if (i2c_write( fd, st.hw->addr, st.reg->bank_sel, 2, tmp))
        return -1;
    if (i2c_write( fd, st.hw->addr, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
        ...
        unsigned char *data)
{
    ...
    if (i2c_write( fd, st.hw->addr, st.reg->bank_sel, 2, tmp))
        return -1;
    if (i2c_read( fd, st.hw->addr, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
...
#define LOAD_CHUNK  (16)
    ...
    for (ii = 0; ii < length; ii += this_write) {
        ...
        if (mpu_write_mem(fd, ii, this_write, (unsigned char*)&firmware[ii]))
            return -1;
        if (mpu_read_mem(fd, ii, this_write, cur))
            return -1;
    ...
    if (i2c_write( fd, st.hw->addr, st.reg->prgm_start_h, 2, tmp))
        return -1;
...
int mpu_set_dmp_state(int fd, unsigned char enable)
{
    ...
    if (enable) {
        ...
        mpu_set_sample_rate(fd, st.chip_cfg.dmp_sample_rate);
        /* Remove FIFO elements. */
        tmp = 0;
        i2c_write( fd, st.hw->addr, 0x23, 1, &tmp);
...
static int setup_compass(int fd)
{
    ...
    for (akm_addr = 0x0C; akm_addr <= 0x0F; akm_addr++) {
        int result;
        result = i2c_read( fd, akm_addr, AKM_REG_WHOAMI, 1, data);
    ...
    if (i2c_write( fd, st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, data))
        return -1;
    delay_ms(1);
    ...
    if (i2c_read( fd, st.chip_cfg.compass_addr, AKM_REG_ASAX, 3, data))
        return -1;
    ...
    if (i2c_write( fd, st.hw->addr, st.reg->s0_addr, 1, data))
        return -1;

    /* Compass reads start at this register. */
    data[0] = AKM_REG_ST1;
    if (i2c_write( fd, st.hw->addr, st.reg->s0_reg, 1, data))
        return -1;

    /* Enable slave 0, 8-byte reads. */
    data[0] = BIT_SLAVE_EN | 8;
    if (i2c_write( fd, st.hw->addr, st.reg->s0_ctrl, 1, data))
        return -1;

    /* Slave 1 changes AKM measurement mode. */
    data[0] = st.chip_cfg.compass_addr;
    if (i2c_write( fd, st.hw->addr, st.reg->s1_addr, 1, data))
        return -1;

    /* AKM measurement mode register. */
    data[0] = AKM_REG_CNTL;
    if (i2c_write( fd, st.hw->addr, st.reg->s1_reg, 1, data))
        return -1;

    /* Enable slave 1, 1-byte writes. */
    data[0] = BIT_SLAVE_EN | 1;
    if (i2c_write( fd, st.hw->addr, st.reg->s1_ctrl, 1, data))
        return -1;
    ...
    data[0] = 0x03;
    if (i2c_write( fd, st.hw->addr, st.reg->i2c_delay_ctrl, 1, data))
        return -1;

#ifdef MPU9150
    /* For the MPU9150, the auxiliary I2C bus needs to be set to VDD. */
    data[0] = BIT_I2C_MST_VDDIO;
    if (i2c_write( fd, st.hw->addr, st.reg->yg_offs_tc, 1, data))
        return -1;
#endif
...
#ifdef AK89xx_BYPASS
    if (i2c_read( fd, st.chip_cfg.compass_addr, AKM_REG_ST1, 8, tmp))
        return -1;
    tmp[8] = AKM_SINGLE_MEASUREMENT;
    if (i2c_write( fd, st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp+8))
        return -1;
#else
    if (i2c_read( fd, st.hw->addr, st.reg->raw_compass, 8, tmp))
        return -1;
#endif
    ...
    unsigned short lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        if (i2c_write( fd, st.hw->addr, st.reg->user_ctrl, 3, data))
            goto lp_int_restore;

        /* Set motion threshold. */
        data[0] = thresh_hw;
        if (i2c_write( fd, st.hw->addr, st.reg->motion_thr, 1, data))
            goto lp_int_restore;
        ...
        if (i2c_write( fd, st.hw->addr, st.reg->lp_accel_odr, 1, data))
            goto lp_int_restore;

        /* Enable motion interrupt (MPU6500 version). */
        data[0] = BITS_WOM_EN;
        if (i2c_write( fd, st.hw->addr, st.reg->accel_intel, 1, data))
            goto lp_int_restore;
        /* Enable LP ACCEL mode, ACCEL_FCHOICE_B=1*/
        if (i2c_read( fd, st.hw->addr, st.reg->accel_cfg2, 1, temp))
        	return -1;
...
lp_int_restore:
    ...
    st.chip_cfg.fifo_enable = 0xFF;
    st.chip_cfg.clk_src = INV_CLK_PLL;
    mpu_set_sensors(fd, st.chip_cfg.cache.sensors_on);
    mpu_set_gyro_fsr(fd, st.chip_cfg.cache.gyro_fsr);
    mpu_set_accel_fsr(fd, st.chip_cfg.cache.accel_fsr);
    mpu_set_lpf(fd, st.chip_cfg.cache.lpf);
    mpu_set_sample_rate(fd, st.chip_cfg.cache.sample_rate);
    mpu_configure_fifo(fd, st.chip_cfg.cache.fifo_sensors);
